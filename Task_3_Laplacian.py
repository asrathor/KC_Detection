...
def cursor_detection(img,img2_gray,img_cursor):
    ...
    imageGray = cv2.Laplacian(imageGray,cv2.CV_8U,3)
    templateGray = cv2.Laplacian(templateGray,cv2.CV_8U,3)
    imageGray = cv2.convertScaleAbs(imageGray)
    templateGray = cv2.convertScaleAbs(templateGray)
